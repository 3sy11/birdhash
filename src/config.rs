...
impl AppConfig {
    ...
    pub fn hits_bf_csv_path(&self) -> PathBuf {
        self.results_dir().join("hits_bf.csv")
    }
    ...
    pub fn derivation_candidates_path(&self) -> PathBuf {
        self.assets_dir
            .join("derivation_candidates")
            .join("derivation_candidates.txt")
    }
    pub fn generator_seed_path(&self) -> PathBuf {
        self.assets_dir.join("generator_seed.key")
    }
    ...
    pub fn load(path: &Path) -> Self {
        ...
        if let Some(f) = toml.fetcher {
            if let Some(ref u) = f.rpc_url {
                cfg.rpc_url = Some(u.clone());
            }
            if let Some(ref urls) = f.rpc_urls {
                if !urls.is_empty() {
                    cfg.rpc_urls = urls.clone();
                }
            }
            if cfg.rpc_urls.is_empty() {
                if let Some(ref u) = f.rpc_url {
                    cfg.rpc_urls = vec![u.clone()];
                }
            }
            if let Some(b) = f.batch_size {
                if b > 0 {
                    cfg.rpc_batch_size = b;
                }
            }
            if let Some(t) = f.timeout_secs {
                if t > 0 {
                    cfg.rpc_timeout_secs = t;
                }
            }
